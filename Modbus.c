...
#ifndef ENABLE_MODBUS_USART_DMA
...
void ModbusInit(modbusHandler_t * modH)
{
  ...
  if (numberHandlers < MAX_M_HANDLERS)
  {
	  ...
	  else if (modH->uModbusType == MB_MASTER)
	  {
		  ...
		  {
			  LOG_E("Failed to create Modbus timeout timer, check heap and stack size\n");
			  return;
		  }
		  ...
		  {
			  LOG_E("Failed to create Modbus telegram queue, check heap and stack size\n");
			  return;
		  }
	  ...
	  {
		  LOG_E("Modbus type not supported, choose a valid Type\n");
		  return;
	  }

	  if  (modH->myTaskModbusAHandle == NULL)
	  {
		  LOG_E("Failed to create Modbus task, check heap and stack size\n");
		  return;
	  }
	  ...
	  {
		  LOG_E("Failed to create T35 timer, check heap and stack size\n");
		  return;
	  }
	  ...
	  {
		  LOG_E("Failed to create Modbus semaphore, check heap and stack size\n");
		  return;
	  }
  ...
  {
	  LOG_E("No more Modbus handlers supported, increase MAX_M_HANDLERS\n");
	  return;
  }
...
void ModbusStart(modbusHandler_t * modH)
{
	...
	{
		LOG_E("Invalid hardware type selected\n");
		return;
	}

	if (modH->xTypeHW == USART_HW_DMA && ENABLE_MODBUS_USART_DMA == 0  )
	{
		LOG_E("To use USART_HW_DMA you need to enable it in the ModbusConfig.h file\n");
		return;
	}
	...
	if (modH->xTypeHW == USART_HW || modH->xTypeHW ==  USART_HW_DMA )
	{
          ...
          {
          	LOG_E("Define the DATA pointer shared through Modbus for slave mode\n");
          	return;
          }
          ...
          if(HAL_UART_Receive_IT(modH->port, &modH->dataRX, 1) != HAL_OK)
          {
                while(1)
                {
                    LOG_E("error in your initialization code");
					HAL_Delay(1000);
                 }
          }
  ...
  for(;;)
  {
   ...
   if(modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA)
   {
	  ...
	  if (getRxBuffer(modH) == ERR_BUFF_OVERFLOW)
	  {
	      modH->i8lastError = ERR_BUFF_OVERFLOW;
	   	  modH->u16errCnt++;
		  LOG_E("Buffer overflow, frame too large\n");
		  continue;
	  }
...
void ModbusQuery(modbusHandler_t * modH, modbus_t telegram )
{
	...
	xQueueSendToBack(modH->QueueTelegramHandle, &telegram, 0);
		...
		LOG_E("A slave cannot send queries as a master\n");
